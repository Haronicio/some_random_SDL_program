...
t[0].vitesseY = 0;
t[1].vitesseX = 0;
t[1].vitesseY = -0.5; //1.0 1.5 1.7 si la masse est 1000 <--> 1    si masse 1000000 <---> 1 ~ 80.1 trajectoire terre (avec saison)
...
while (1){
	...
	for ( i = 0; i < N; ++i)
	{
		...
		distanceX = t[0].x - t[1].x ;//ici mettre une fonction de la distance entre tous les objets
		distanceY = t[0].y - t[1].y ;
		//printf("dx %f\n",distanceX );
		...
		printf("acc X =%f acc Y =%f\n",t[1].accelerationX,t[1].accelerationY );
		//t[1].y += 1 * -0.001;
		...
		//printf("i = %d acc %e x %f y %f\n",i,t[1].acceleration,t[1].x, t[1].y );	
