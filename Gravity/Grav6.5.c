	...
	for ( i = 0; i < N; ++i)
	{
		...
		for ( j = 0; j < N; ++j)
		{
			...
			if (i != j )
			{
				...
				t[i].vitesseX  += t[i].accelerationX * (SDL_GetTicks() / CLOCKS_PER_SEC);//nombre de tick écou_lé depuis le début / nombre de tick toute les secondes
				t[i].vitesseY  += t[i].accelerationY  * (SDL_GetTicks() / CLOCKS_PER_SEC);
