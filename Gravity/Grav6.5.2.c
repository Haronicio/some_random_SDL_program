	...
	while(SDL_PollEvent(&evenement))
	{
		...
		if (evenement.type == SDL_MOUSEBUTTONUP)
		{
			switch (appui)
			{
				...
				case 1 : SDL_GetRelativeMouseState(&x,&y);N++;/*realloc(t, N * taille_struct);*/ appui = 0 ;printf("%d %d \n",x,y );
				t[N-1].x = xprim;
				t[N-1].y = yprim;
				t[N-1].masse = 0.001;
				t[N-1].vitesseX = -0.7;//(x - xprim) /100;
				t[N-1].vitesseY =-0.5;//(y - yprim) /100;
				t[N-1].force = 0;
				t[N-1].acceleration = 0;
				printf("nouvel objet place %d\n",&t[N-1] );
	...
	for ( i = 0; i < N; ++i)
	{
		...
		for ( j = 0; j < N; ++j)
		{
			...
			if (i != j )
			{
				...
				distanceTOTALE = ((distanceX * distanceX) + (distanceY * distanceY));

				t[i].force = ( G * t[i].masse * t[j].masse/*massetotale(t)*/ ) / distanceTOTALE; //massetotal = masse de tout les obets 
				...
				t[i].vitesseX  += t[i].acceleration * (distanceX/sqrt(distanceTOTALE));
				t[i].vitesseY  += t[i].acceleration  * (distanceY/sqrt(distanceTOTALE));
...
moin de calcul c'est mieux :
distanceTOTALE = sqrt((distanceX * distanceX) + (distanceY * distanceY))

Or 
t[i].force = ( G * t[i].masse * t[j].masse ) / ( distanceTOTALE * distanceTOTALE)

Donc
distanceTOTALE = (sqrt((distanceX * distanceX) + (distanceY * distanceY))) * (sqrt((distanceX * distanceX) + (distanceY * distanceY)))
				= sqrt((distanceX * distanceX) + (distanceY * distanceY) * (distanceX * distanceX) + (distanceY * distanceY)))
